/**
 * Script pour insérer les badges mensuels dans la base de données
 * 
 * Usage depuis la console du navigateur:
 * 1. Ouvrez la console (F12)
 * 2. Importez et exécutez:
 *    const { seedMonthlyBadges } = await import('/src/scripts/seedMonthlyBadges.ts');
 *    await seedMonthlyBadges();
 * 
 * OU depuis le code:
 *    import { seedMonthlyBadges } from '@/scripts/seedMonthlyBadges';
 *    await seedMonthlyBadges();
 */
...
if (typeof window !== 'undefined') {
  (window as any).seedMonthlyBadges = seedMonthlyBadges;
}

// Liste des badges mensuels à insérer
const monthlyBadges = [
  // ================== LEARNING ==================
...
export async function seedMonthlyBadges() {
  console.log('🚀 Début de l\'insertion des badges mensuels...\n');
  ...
  try {
    ...
    if (checkError) {
      if (checkError.code === '42P01' || checkError.message?.includes('relation') || checkError.message?.includes('does not exist')) {
        console.error('❌ La table "badges" n\'existe pas!');
        console.log('💡 Veuillez d\'abord exécuter la migration SQL dans Supabase SQL Editor:');
        console.log('   supabase/migrations/20250104000002_add_monthly_badges.sql\n');
        return { success: false, error: checkError };
      } else {
        console.warn('⚠️  Erreur lors de la vérification:', checkError.message);
      }
    }

    // Supprimer les anciens badges mensuels (optionnel - commentez cette ligne si vous voulez garder les anciens)
    console.log('🗑️  Suppression des anciens badges mensuels...');
    ...
    if (deleteError) {
      ...
      // Continue anyway, peut-être qu'il n'y avait pas de badges à supprimer
    } else {
      console.log('✅ Anciens badges mensuels supprimés\n');
    }

    // Insérer les nouveaux badges
    console.log(`📦 Insertion de ${monthlyBadges.length} badges mensuels...\n`);
    
    const { data, error } = await supabase
      .from('badges')
      .insert(monthlyBadges)
      .select();
    ...
    console.log(`✅ ${data?.length || monthlyBadges.length} badges mensuels insérés avec succès!\n`);

    // Afficher un résumé par catégorie
    const categoryCounts = monthlyBadges.reduce((acc, badge) => {
    ...
    return { 
      success: true, 
      inserted: data?.length || monthlyBadges.length,
